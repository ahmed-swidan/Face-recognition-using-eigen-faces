...
accuracy = np.sum(test_training_labels[0:n] == predictions[0:n]) / len(test_training_labels[0:n]) * 100
face_accuracy = np.sum(test_training_labels[0:20] == predictions[0:20]) / len(test_training_labels[0:20]) * 100
nonface_accuracy = np.sum(test_training_labels[20:n] == predictions[20:n]) / len(test_training_labels[20:n]) * 100
print("Using LDA with k-nearest neighbour of value 1 using ", (n-20)/20 *100, "% of the nonfaces dataset")
